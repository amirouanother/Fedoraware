...
void CCameraWindow::SpyCamera(CBaseEntity* pLocal)
{
	// Spy camera
...
void CCameraWindow::TeleCamera(CBaseEntity* pLocal)
{
	// Teleporter camera
	...
	for (const auto& pBuilding : buildings)
	{
		...
		// Is it a teleporter?
		if (nType != EBuildingType::TELEPORTER || building->GetObjectMode() != 0) { continue; }

		// Get the teleporter exit
		Vec3 tpExit;
		if (!Utils::GetTeleporterExit(building->GetOwner()->GetIndex(), &tpExit)) { continue; }
		...
		if (Vars::Visuals::CameraMode.Value == 4)
		{
			...
			if (Utils::W2S(building->GetAbsOrigin(), vScreen))
			{
				...
				ViewRect.w = static_cast<int>(width);
				ViewRect.h = static_cast<int>(height);
				ViewRect.x = static_cast<int>(vScreen.x - width * 0.5);
				ViewRect.y = static_cast<int>(vScreen.y - height);
			}
			else
			{
				CanDraw = true;
...
void CCameraWindow::RenderCustomView(void* ecx, const CViewSetup& pViewSetup, ITexture* pTexture)
{
	...
	if (static auto renderViewHook = g_HookManager.GetMapHooks()["ViewRender_RenderView"])
	{
		renderViewHook->Original<void(*)(void*, const CViewSetup&, int, int)>()(ecx, pViewSetup, VIEW_CLEAR_COLOR | VIEW_CLEAR_DEPTH | VIEW_CLEAR_STENCIL, RENDERVIEW_UNSPECIFIED);
	}
